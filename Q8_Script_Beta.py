...
#prepare lists
alpha_list = [] #to check levels of alpha parameter in beta distribution
beta_dis_list = [] #to check levels of beta parameter in beta distribution
beta_list = [] #list of drawn samples of beta from Beta Distribution
...
#parameters to dynamically change Alpha and Beta
delta = 0.8
theta = 20

#Markov-Chain Monte-Carlo with Metropolis-Hastings algorithm
for i in range(1, 10001):
    #select beta from 'q' distribution based on old value of beta
    #parameters 'alpha' and 'beta_dis' dependent on last accepted beta
    alpha = max(delta + theta*beta_list[i-1],1)
    beta_dis = max(delta + theta *(1-beta_list[i-1]), 1)

    #append alpha and beta values to list
    alpha_list.append(alpha)
    beta_dis_list.append(beta_dis)

    #draw sample of beta
    beta = np.random.beta(alpha, beta_dis)
...
#Plot of Alpha's and Beta's Used as Parameters for final Beta Distribution to sample beta from
#plt.plot(alpha_list, beta_dis_list, "ro")
#plt.title("Combination of Alpha and Beta parameters used for Distribution", fontsize=12, fontweight=0)
#plt.xlabel("Alpha")
#plt.ylabel("Beta")
